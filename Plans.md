...
| 5.2  | Publish a real inside-camera handoff after a successful entry animation status. [integration] [tdd:required]                              | A genuine recognized entry reads its configured map start and publishes one idempotent handoff; exit, sighting, and duplicate transitions publish none; external failure does not roll back the visit. | 5.1     | cc:done |
