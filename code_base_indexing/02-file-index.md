...
| `src/app/api/face/session/route.ts`         | Route       | Auth + same-origin; create/reuse one liveness session → `{ sessionId }` (`409` already registered)                                              | `POST`                                       |
| `src/app/api/face/result/route.ts`          | Route       | Auth + same-origin; read one owned result (`Get` ×1, no polling) → `{ outcome, confidence? }`                                                   | `POST`                                       |
...
| `src/components/face-liveness-registration.tsx` | Client `UC` | Face registration flow; creates one session on explicit start, runs `FaceLivenessDetectorCore` with a custom credential provider, reads the backend decision once (one retry) | `FaceLivenessRegistration`                            |
...
| File               | Kind   | Purpose                                                                                                               | Key exports                                                                                                                                                                 |
| ------------------ | ------ | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/db/schema.ts` | Schema | Tables `shelves`, `products`, `shelf_products`, `users`, `sessions`, `face_liveness_attempts` + `auth_method`/`face_enrollment_status`/`liveness_attempt_status` enums + relations + inferred types | `shelves`, `products`, `shelfProducts`, `users`, `sessions`, `faceLivenessAttempts`, `authMethodEnum`, `faceEnrollmentStatusEnum`, `livenessAttemptStatusEnum`, `*Relations`, types `Shelf`/`Product`/`ShelfProduct`/`User`/`NewUser`/`Session`/`FaceLivenessAttempt`/`NewFaceLivenessAttempt`/`AuthMethod`/`FaceEnrollmentStatus`/`LivenessAttemptStatus` |
...
| `src/services/face-enrollment.service.ts` | `SO` | Liveness business logic: credential bridge (Google→Cognito), create/reuse one attempt, read one result, mark user registered on accepted decision | `faceEnrollmentService` singleton, `FaceAlreadyRegisteredError`, `LivenessAttemptNotFoundError`, `CredentialBridgeError`, type `DetectorCredentials` |
...
| `src/lib/liveness-state.ts`         | Pure util   | AWS/DB-free liveness state machine (decision threshold, attempt reuse, status mapping) for unit tests | `decideLivenessOutcome`, `isAttemptReusable`, `enrollmentStatusForOutcome`, `attemptStatusForOutcome`, types `LivenessOutcome`/`LivenessDecision`/`RekognitionLivenessStatus` |
