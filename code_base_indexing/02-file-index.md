...
| `src/app/api/auth/signin/google/route.ts` | Route | Builds Google OAuth URL + redirects; logs flow start | `GET` |
| `src/app/api/auth/callback/google/route.ts` | Route | OAuth callback; exchanges code, upserts user, opens session, sets cookie; verbose `console.log` monitoring | `GET` |
| `src/app/api/auth/signout/route.ts` | Route | Deletes session + clears cookie → `/signin` | `GET`, `POST` |
...
| `src/services/user.service.ts` | `SO` | `UserService.upsertOAuthUser`, `createSession`, `getUserBySession`, `deleteSession` | `userService` singleton, type `Session` |
...
| `src/lib/auth.ts` | `SO` | Secure DAL: `getCurrentUser()` (React-cached, DB-backed) | `getCurrentUser` |
| `src/lib/auth-shared.ts` | Shared | Edge-safe constants/helpers (no `server-only`) for proxy + DAL | `SESSION_COOKIE`, `SIGN_IN_PATH`, `PUBLIC_PATHS`, `sessionCookieOptions` |
